...
[🔗 PMC2064100](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2064100/)
...
| 数据结构类型       | 时间特征                          | 是否追踪     | 应用场景示例               | 常用统计分析方法                              |
|--------------------|-----------------------------------|-------------|---------------------------|---------------------------------------------|
| 横断面研究         | 单时间点<br>（无前后）           | ❌ 无追踪    | 现况调查、患病率估计       | t检验、卡方检验、单因素/多因素Logistic回归  |
| 病例对照研究       | 单时间点<br>（回顾性暴露）       | ❌ 不追踪    | 病因探索、罕见病研究       | OR估计、Logistic回归、条件Logistic回归      |
| 随机对照试验（RCT）| 多时间点<br>（前瞻性：基线+随访）| ✅ 干预后追踪 | 药物疗效、治疗比较         | t检验、ANCOVA、Cox回归（生存结局）          |
| 队列研究           | 多时间点<br>（前瞻或回顾）       | ✅ 长期追踪  | 危险因素评估、预后分析     | Kaplan-Meier、Cox回归、Poisson回归          |
| 登记研究           | 多时间点<br>（通常前瞻）         | ✅ 长期追踪  | 真实世界数据、药物监测     | 描述统计、倾向评分匹配/加权、Logistic/Cox回归 |
